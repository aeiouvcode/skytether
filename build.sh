#!/bin/sh
# Export the web build and add a strict Content-Security-Policy (no network, no third-party code).
set -e
GODOT=${GODOT:-godot}
mkdir -p export
...
p = 'export/index.html'
h = open(p).read()
hashes = ["'sha256-" + base64.b64encode(hashlib.sha256(m.group(1).encode()).digest()).decode() + "'"
...
open(p, 'w').write(h.replace('<head>', '<head>\n' + meta, 1))
